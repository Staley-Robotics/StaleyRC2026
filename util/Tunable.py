from enum import Enum, auto
import typing as tp

from ntcore.util import ntproperty
ntproperty

from ntcore import NetworkTable, NetworkTableInstance, NetworkTableEntry

'''
reference:
ntproperty definition
https://github.com/Mechanical-Advantage/RobotCode2026Public/blob/main/src/main/java/org/littletonrobotics/frc2026/util/LoggedTunableNumber.java
'''

__all__ = ["TunableSystemModes", "Tunable"]

class TunableSystemModes:
    '''Modes for the tunable system. Dictates what created Tunables will be interactable from the NetworkTables'''
    FULL=auto()
    '''Enables all values as changeable from NetworkTables'''
    LIMITED=auto()
    '''Enables only values marked Limited as changeable from NetworkTables'''
    REQUIRED=auto()
    '''Enables only values marked Required as changeable from NetworkTables'''

class Tunable:

    default_key = "/Tuning"

    ntInst: NetworkTableInstance = NetworkTable.getInstance

    def __init__(self, key:str, defaultValue:tp.Any, persistent:bool=False, writeDefault:bool=False):
        
        # Create nt entry key
        # if the user assigned key starts with "/" start the filepath in the base directory, else put it in "/Tuning"
        entry_key = ""

        if key[0] == "/":
            entry_key += key
        else:
            entry_key += self.default_key
            entry_key += key[1:]

        entry_key += key

        # create the nt entry object
        self.entry: NetworkTableEntry = self.ntInst.getEntry(entry_key)


        if persistent:
            writeDefault = False
            self.entry.setPersistent()
        
        self.write_default = writeDefault
        self.default_val = defaultValue

        self.reset()

    def reset(self):

        if self.write_default:
            self.entry.setValue(self.default_val)
        else:
            self.entry.setDefaultValue(self.default_val)
        
    
    def get(self):
        return self.entry.value # i dont know waht this does
    
    def set(self, val:tp.Any):
        self.entry.setValue(val)